...
describe("One opportunity ten investors", function () {
	let dygnifyConfig,
		seniorPool,
		lpToken,
		borrowerContract,
	...
	beforeEach(async () => {
		...
		investorUser = accounts[5];
		investorUser2 = accounts[6];
		investorUser3 = accounts[7];
		investorUser4 = accounts[8];
		investorUser5 = accounts[9];
		investorUser6 = accounts[10];
		investorUser7 = accounts[11];
		investorUser8 = accounts[12];
		investorUser9 = accounts[13];
		investorUser10 = accounts[14];
		underwriter = accounts[15];
		...
		await usdcToken.transfer(investorUser5.address, "80000000000000000");
		await usdcToken.transfer(investorUser6.address, "80000000000000000");
		await usdcToken.transfer(investorUser7.address, "80000000000000000");
		await usdcToken.transfer(investorUser8.address, "80000000000000000");
		await usdcToken.transfer(investorUser9.address, "80000000000000000");
		await usdcToken.transfer(investorUser10.address, "80000000000000000");
	...
	// Ten different investors invested in 1 opportunity
	const oneOpportunityTenInvestor = ({
		opportunityName,
		opportunityInfo,
		loanType,
		loanAmount,
		...
		async function () {
			...
			const seniorAmount = ((loanAmount * 0.8) / 4).toString();
			// deposit 20% of AMOUNT into junior Pool
			const amountToJunior = (loanAmount - seniorAmount * 4).toString();
			await usdcToken
				.connect(underwriter)
			...
			await usdcToken
				...
				.approve(seniorPool.address, seniorAmount);
			await usdcToken
				.connect(investorUser5)
				.approve(seniorPool.address, seniorAmount);
			await usdcToken
				.connect(investorUser6)
				.approve(seniorPool.address, seniorAmount);
			await usdcToken
				.connect(investorUser7)
				.approve(seniorPool.address, seniorAmount);
			await usdcToken
				.connect(investorUser8)
				.approve(seniorPool.address, seniorAmount);
			await usdcToken
				.connect(investorUser9)
				.approve(seniorPool.address, seniorAmount);
			await usdcToken
				.connect(investorUser10)
				.approve(seniorPool.address, seniorAmount);
			...
			await seniorPool.connect(investorUser5).stake(seniorAmount);
			await seniorPool.connect(investorUser6).stake(seniorAmount);
			await seniorPool.connect(investorUser7).stake(seniorAmount);
			await seniorPool.connect(investorUser8).stake(seniorAmount);
			await seniorPool.connect(investorUser9).stake(seniorAmount);
			await seniorPool.connect(investorUser10).stake(seniorAmount);
			...
			[withdrawableAmt] = await seniorPool
				.connect(investorUser5)
				.getUserInvestment();
			beforeBalance = await usdcToken.balanceOf(investorUser5.address);
			await seniorPool.connect(investorUser5).withdrawWithLP(withdrawableAmt);
			afterBalance = await usdcToken.balanceOf(investorUser5.address);
			expect(afterBalance.sub(beforeBalance)).to.equal(expectedUSDCAmount);
			...
			[withdrawableAmt] = await seniorPool
				.connect(investorUser6)
				.getUserInvestment();
			beforeBalance = await usdcToken.balanceOf(investorUser6.address);
			await seniorPool.connect(investorUser6).withdrawWithLP(withdrawableAmt);
			afterBalance = await usdcToken.balanceOf(investorUser6.address);
			expect(afterBalance.sub(beforeBalance)).to.equal(expectedUSDCAmount);
			...
			[withdrawableAmt] = await seniorPool
				.connect(investorUser7)
				.getUserInvestment();
			beforeBalance = await usdcToken.balanceOf(investorUser7.address);
			await seniorPool.connect(investorUser7).withdrawWithLP(withdrawableAmt);
			afterBalance = await usdcToken.balanceOf(investorUser7.address);
			expect(afterBalance.sub(beforeBalance)).to.equal(expectedUSDCAmount);
			...
			[withdrawableAmt] = await seniorPool
				.connect(investorUser8)
				.getUserInvestment();
			beforeBalance = await usdcToken.balanceOf(investorUser8.address);
			await seniorPool.connect(investorUser8).withdrawWithLP(withdrawableAmt);
			afterBalance = await usdcToken.balanceOf(investorUser8.address);
			expect(afterBalance.sub(beforeBalance)).to.equal(expectedUSDCAmount);
			...
			[withdrawableAmt] = await seniorPool
				.connect(investorUser9)
				.getUserInvestment();
			beforeBalance = await usdcToken.balanceOf(investorUser9.address);
			await seniorPool.connect(investorUser9).withdrawWithLP(withdrawableAmt);
			afterBalance = await usdcToken.balanceOf(investorUser9.address);
			expect(afterBalance.sub(beforeBalance)).to.equal(expectedUSDCAmount);
			...
			[withdrawableAmt] = await seniorPool
				.connect(investorUser10)
				.getUserInvestment();
			beforeBalance = await usdcToken.balanceOf(investorUser10.address);
			await seniorPool.connect(investorUser10).withdrawWithLP(withdrawableAmt);
			afterBalance = await usdcToken.balanceOf(investorUser10.address);
			expect(afterBalance.sub(beforeBalance)).to.equal(expectedUSDCAmount);
		};
