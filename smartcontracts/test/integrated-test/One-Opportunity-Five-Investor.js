...
describe("Dygnify integrated test", function () {
  ...
  beforeEach(async () => {
    ...
    await usdcToken.transfer(investors[0].address, "8000000000000000000");
    await usdcToken.transfer(investors[1].address, "8000000000000000000");
    await usdcToken.transfer(investors[2].address, "8000000000000000000");
    await usdcToken.transfer(investors[3].address, "8000000000000000000");
    await usdcToken.transfer(investors[4].address, "8000000000000000000");
  ...
  describe("One Opportunity Five Investor", function () {
    const basicDygnifyEndToEndFlow = ({
      ...
      async function () {
        ...
        const seniorAmount = ethers.BigNumber.from((loanAmount * 0.8).toString());
        ...
        await seniorPool.connect(investors[0]).stake(seniorAmount.div(5));
        ...
        await seniorPool.connect(investors[1]).stake(seniorAmount.div(10));
        ...
        await seniorPool.connect(investors[2]).stake(seniorAmount.div(20).mul(3));
        ...
        await seniorPool.connect(investors[3]).stake(seniorAmount.div(4));
        ...
        await seniorPool.connect(investors[4]).stake(seniorAmount.div(20).mul(6));
        ...
        // for 1st investor
        const expectedUSDCAmount0 = getUSDCAmountFromShares(
          seniorAmount.div(5),
          sharePrice.toString(),
          lpMantissa().toString()
        );

        const [withdrawableAmt0] = await seniorPool
          .connect(investors[0])
          .getUserInvestment();

        beforeBalance = await usdcToken.balanceOf(investors[0].address);
        await seniorPool.connect(investors[0]).withdrawWithLP(withdrawableAmt0);
        afterBalance = await usdcToken.balanceOf(investors[0].address);

        expect(afterBalance.sub(beforeBalance)).to.equal(expectedUSDCAmount0);

        // for 2nd investor

        sharePrice = await seniorPool.sharePrice();
        const expectedUSDCAmount1 = getUSDCAmountFromShares(
          seniorAmount.div(10),
          sharePrice.toString(),
          lpMantissa().toString()
        );

        const [withdrawableAmt1] = await seniorPool
          .connect(investors[1])
          .getUserInvestment();

        beforeBalance = await usdcToken.balanceOf(investors[1].address);
        await seniorPool.connect(investors[1]).withdrawWithLP(withdrawableAmt1);
        afterBalance = await usdcToken.balanceOf(investors[1].address);

        expect(afterBalance.sub(beforeBalance)).to.equal(expectedUSDCAmount1);

        // for 3nd investor

        sharePrice = await seniorPool.sharePrice();
        const expectedUSDCAmount2 = getUSDCAmountFromShares(
          seniorAmount.div(20).mul(3),
          sharePrice.toString(),
          lpMantissa().toString()
        );

        const [withdrawableAmt2] = await seniorPool
          .connect(investors[2])
          .getUserInvestment();

        beforeBalance = await usdcToken.balanceOf(investors[2].address);
        await seniorPool.connect(investors[2]).withdrawWithLP(withdrawableAmt2);
        afterBalance = await usdcToken.balanceOf(investors[2].address);

        expect(afterBalance.sub(beforeBalance)).to.equal(expectedUSDCAmount2);

        // for 4th investor

        sharePrice = await seniorPool.sharePrice();
        const expectedUSDCAmount3 = getUSDCAmountFromShares(
          seniorAmount.div(4),
          sharePrice.toString(),
          lpMantissa().toString()
        );

        const [withdrawableAmt3] = await seniorPool
          .connect(investors[3])
          .getUserInvestment();

        beforeBalance = await usdcToken.balanceOf(investors[3].address);
        await seniorPool.connect(investors[3]).withdrawWithLP(withdrawableAmt3);
        afterBalance = await usdcToken.balanceOf(investors[3].address);

        expect(afterBalance.sub(beforeBalance)).to.equal(expectedUSDCAmount3);

        // for 5th investor

        sharePrice = await seniorPool.sharePrice();
        const expectedUSDCAmount4 = getUSDCAmountFromShares(
          seniorAmount.div(20).mul(6),
          sharePrice.toString(),
          lpMantissa().toString()
        );

        const [withdrawableAmt4] = await seniorPool
          .connect(investors[4])
          .getUserInvestment();

        beforeBalance = await usdcToken.balanceOf(investors[4].address);
        await seniorPool.connect(investors[4]).withdrawWithLP(withdrawableAmt4);
        afterBalance = await usdcToken.balanceOf(investors[4].address);

        expect(afterBalance.sub(beforeBalance)).to.equal(expectedUSDCAmount4);
      };
