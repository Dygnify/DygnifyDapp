...
describe("Three opportunities two investors", function () {
	let dygnifyConfig,
		seniorPool,
		lpToken,
		borrowerContract,
	...
	// Two investors invested in all three opportunities
	const threeOpportunityTwoInvestor = ({
		opportunityName,
		opportunityInfo,
		loanType,
		loanAmount,
		...
		async function () {
			...
			let seniorAmountBn = loanAmountBn.mul(80).div(100).div(2);
			let seniorAmount = seniorAmountBn.toString();
			// deposit 20% of AMOUNT into junior Pool
			let amountToJuniorBn = loanAmountBn.sub(seniorAmountBn.mul(2));
			...
			await usdcToken
				...
				.approve(seniorPool.address, seniorAmount * 2);
			await usdcToken
			...
			expect(afterBalance.sub(beforeBalance)).to.equal(expectedUSDCAmount * 3);
