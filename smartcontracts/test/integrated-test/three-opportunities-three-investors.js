...
const offsetBn = ethers.BigNumber.from(10);
...
describe("Three opportunities three investors", function () {
	let dygnifyConfig,
		seniorPool,
		lpToken,
		borrowerContract,
	...
	// Three investors invested in all three opportunities
	const threeOpportunityThreeInvestor = ({
		opportunityName,
		opportunityInfo,
		loanType,
		loanAmount,
		...
		async function () {
			...
			// Assign due diligence expert
			// call assignUnderwriters function
			await opportunityOrigination.assignUnderwriters(ID3, underwriter.address);
			await opportunityOrigination.connect(underwriter).voteOpportunity(ID3, 2);
			...
			let opportunity3 = await opportunityOrigination.opportunityToId(ID3);
			const poolAddress1 = opportunity1.opportunityPoolAddress;
			const poolAddress2 = opportunity2.opportunityPoolAddress;
			const poolAddress3 = opportunity3.opportunityPoolAddress;
			...
			let opportunityPool3 = await ethers.getContractAt(
				"OpportunityPool",
				poolAddress3
			);
			let loanAmountBn = ethers.BigNumber.from(loanAmount);
			let seniorAmountBn = loanAmountBn.mul(80).div(100).div(3);
			let seniorAmount = seniorAmountBn.toString();
			let seniorExtraAmount = seniorAmountBn.add(2).toString();
			// deposit 20% of AMOUNT into junior Pool
			let amountToJuniorBn = loanAmountBn.sub(loanAmountBn.mul(80).div(100));
			let amountToJunior = amountToJuniorBn.toString();
			...
			await usdcToken
				...
				.approve(seniorPool.address, seniorAmount * 3);
			await usdcToken
				.connect(investorUser2)
				.approve(seniorPool.address, seniorAmount * 3);
			await usdcToken
				.connect(investorUser3)
				.approve(seniorPool.address, seniorAmount * 4);
			...
			await usdcToken
				.connect(underwriter)
				.approve(opportunityPool3.address, amountToJunior);
			await opportunityPool3
				.connect(underwriter)
				.deposit(subpool.JuniorSubpool, amountToJunior);

			await seniorPool.connect(investorUser).stake(seniorAmount);
			await seniorPool.connect(investorUser2).stake(seniorAmount);
			await seniorPool.connect(investorUser3).stake(seniorExtraAmount);
			...
			await seniorPool.approveUSDC(opportunityPool3.address);
			await seniorPool.invest(ID3);
			...
			// Drawdown
			beforeBalance = await usdcToken.balanceOf(borrowerUser.address);
			await opportunityPool3.connect(borrowerUser).drawdown();
			afterBalance = await usdcToken.balanceOf(borrowerUser.address);
			...
			// Opportunity status should marked as drawdown
			opportunity3 = await opportunityOrigination.opportunityToId(ID3);
			expect(opportunity3.opportunityStatus).to.equal(
				OpportunityStatus.Drawndown
			);
			...
			await usdcToken
				.connect(borrowerUser)
				.approve(opportunityPool3.address, "100000000000000000000");
			let totalRepayments3 = (
				await opportunityPool3.totalRepayments()
			).toNumber();
			...
			// Repayment
			for (let i = 0; i < totalRepayments3; i++) {
				await opportunityPool3.connect(borrowerUser).repayment();
			}
			...
			// Opportunity status should marked as repaid
			opportunity3 = await opportunityOrigination.opportunityToId(ID3);
			expect(opportunity3.opportunityStatus).to.equal(OpportunityStatus.Repaid);
			...
			// Underwriter withdraw from OpportunityPool
			beforeBalance = await usdcToken.balanceOf(underwriter.address);
			await opportunityPool3.connect(underwriter).withdrawAll(0);
			afterBalance = await usdcToken.balanceOf(underwriter.address);
			expect(afterBalance.sub(beforeBalance)).to.be.above(0);
			...
			expect(result.sub(expectedUSDCAmount.mul(3))).lte(offsetBn);
			...
			result = afterBalance.sub(beforeBalance);
			expect(result.sub(expectedUSDCAmount.mul(3))).lte(offsetBn);

			sharePrice = await seniorPool.sharePrice();
			investedAmount = seniorExtraAmount;
			expectedUSDCAmount = getUSDCAmountFromShares(
				investedAmount,
				sharePrice.toString(),
				lpMantissa().toString()
