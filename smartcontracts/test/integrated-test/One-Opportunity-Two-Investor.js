...
describe("Dygnify integrated test", function () {
  ...
  beforeEach(async () => {
    ...
    investors = [accounts[3], accounts[4]];
    ...
    //AdjustmentOffset
    await dygnifyConfig.setNumber(6, 115);
  ...
  describe("OneOpportunityTwoInvestor", function () {
    const basicDygnifyEndToEndFlow = ({
      ...
      async function () {
        ...
        const seniorAmount = ethers.BigNumber.from(
          (loanAmount * 0.8).toString()
        );
        // deposit 20% of AMOUNT into junior Pool
        const amountToJunior = ethers.BigNumber.from(
          (loanAmount - seniorAmount).toString()
        );
        await usdcToken
          .connect(underwriter)
        ...
        await seniorPool.connect(investors[1]).stake(seniorAmount.div(100).mul(80));
