...
const ErrorModal = ({ errormsg, setErrormsg }) => {
	...
	const unCatcherror3 = errormsg?.msg?.includes(
		"Same collatoral document is been used"
	);
	const unCatcherror4 = errormsg?.msg?.includes("code=ACTION_REJECTED");
	...
	if (unCatcherror2 && unCatcherror3) {
		setErrormsg({
			status: true,
			msg: "Same collatoral document is been used to create different opportunity",
		});
	}
	if (unCatcherror4) {
		setErrormsg({
			status: true,
			msg: "User rejected to create opportunity",
		});
	}
