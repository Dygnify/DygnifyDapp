...
const EditBorrowerProfile = () => {
    ...
    const [businessIncorporationFiles, setBusinessIncorporationFiles] = useState();
    ...
    const uploadBorrowerData = async () => {
        try {
            ...
            if (businessIncorporationFiles && businessIncorporationFiles.length) {
                businessIncoFilesCID = await uploadFilesToIPFS(businessIncorporationFiles);
            }
            ...
            let borrowerJsonData = { 'companyName': companyName.current.value,
                                    'companyRepName': companyRepName.current.value,  
                                    'companyBio': companyBio.current.value,
                                    'companyLogoCID': logoFileCID,
                                    'kycFile': { 'kycFileName': kycFileName.current.value,
                                                    'kycCID': kycFilesCID },
                                    'businessIdFile': { 'businessIdFileName': bizIdFileName.current.value,
                                                        'businessIdFileCID': businessIdFilesCID },
                                    'businessAddFile': { 'businessAddFileName': bizAddFileName.current.value,
                                                         'businessAddFileCID': businessAddFilesCID },
                                    'businessIncoFile': { 'businessIncoFileName': bizIncoFileName.current.value,
                                                          'businessIncoFileCID': businessIncoFilesCID },
                                    'businessLicFile': { 'businessLicFileName': bizLicFileName.current.value,
        ...
        <div>
            <div className='mb-6'>
                ...
                <div style={{ display: 'flex' }} className='w-full'>
                    <FileFields label='Company Logo' className='w-1/3' onChange={onLogoFileUpload}/>
                    <TextField label='Company Name' placeholder='Enter Company Name' className='w-1/3 ml-2' reference={companyName}/>
                    <TextField label='Company Representative Name' placeholder='Enter Name' className='w-1/3 ml-2' reference={companyRepName}></TextField>
                </div>
                <div>
                    <TextArea label='Bio' placeholder='Summary About the Organization/Company' className='w-full'
                                reference={companyBio}></TextArea>
                </div>
            </div>
            <div className='mb-6'>
            ...
            <div className='mb-6'>
                ...
                <InputGroup caption='Business Identify Proof' onChange={onBusinessIdentityFilesUpload} reference={bizIdFileName} />
                <InputGroup caption='Business Address Proof' onChange={onBusinessAddressFilesUpload} reference={bizAddFileName}/>
                <InputGroup caption='Business Incorporation Proof' onChange={onBusinessIncorporationFilesUpload} reference={bizIncoFileName}/>
                <InputGroup caption='Business License Proof' onChange={onBusinessLicenseFilesUpload} reference={bizLicFileName}/>
            </div>
            <div className='mb-6'>
                <h2 className='text-xl font-medium mb-2'>Socials</h2>
                <div className='w-full' style={{ display: 'flex' }}>
                    <TextField label='Website' placeholder='Enter Website URL' className='w-1/2 mr-8' reference={website}></TextField>
                    <TextField label='Email Address' placeholder='Enter Email Address' className='w-1/2 ml-8' reference={email}></TextField>
                </div>
                <div className='w-full' style={{ display: 'flex' }}>
                    <TextField label='Twitter' placeholder='Enter Twitter URL' className='w-1/2 mr-8' reference={twitter}></TextField>
                    <TextField label='LinkedIn' placeholder='Enter LinkedIn URL' className='w-1/2 ml-8' reference={linkedin}></TextField>
                </div>
            </div>
            <div className='my-10 justify-center' style={{ display: 'flex' }}>
                ...
                <GradientButton className='font-medium ml-4' onClick={uploadBorrowerData} >Save and Exit</GradientButton>
            </div>
        </div>
    );
};
